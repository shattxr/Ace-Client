...
let staticMemory = {
    ...
    // * Disable bots from joining a game quickly
    SPAM_JOIN_PREVENTION: {
        active: true,
        nextJoinTime: 0,
        buildNextJoinTime: () => game.step + (45 + ~~(Math.random() * 45))
    },
...
//     let insideOf = document.querySelector("#insiderunpanel");

//     if (!insideOf) {
//         statusMessage("error", "Failure initializing admin panel")
//     }

//     let div = document.createElement('div');
//     div.innerHTML = `
//         <div id="FL_ADMIN_PANEL" style="position: absolute; bottom: 0; left: 50%; width: 25%; height: 48%; background: red;">
//             hello
//         </div>
//     `

//     console.log(document.firstChild.ownerDocument.children[0].children[1]);
//     document.firstChild.ownerDocument.children[0].children[1].appendChild(div.firstChild);
    ...
    switch (event.name) {
        ...
        case "ship_spawned":
            if (event.ship != null) {
                ...
                if (event.ship.custom._killOnRespawn) {ship.set({kill: true})};
                
                if (game.step < staticMemory.SPAM_JOIN_PREVENTION.nextJoinTime) {
                    let njt = staticMemory.SPAM_JOIN_PREVENTION.buildNextJoinTime()
                    staticMemory.SPAM_JOIN_PREVENTION.nextJoinTime = njt;
                    return kickPlayer(event.ship, "You have joined too fast. Try again");
                }

                staticMemory.SPAM_JOIN_PREVENTION.nextJoinTime = staticMemory.SPAM_JOIN_PREVENTION.buildNextJoinTime();
...
const kickPlayer = (ship, message = "You have been kicked from participating") => {
    ship.set({kill: true});
    ship.gameover({ "": message});
    ship.custom._killOnRespawn = true;
    ship.custom._forbidInput = true;
